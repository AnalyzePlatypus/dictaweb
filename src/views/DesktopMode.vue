    ...
    <main class="main-container">
      <section>
        <form class="form" @submit.prevent>
          ...
          <div class="button-bar flex-horiz justify-between">
            ...
            <button class="button primary" :disabled="formIsEmpty" @click="generatePDF">Generate PDF</button>
          </div>
        </form>
      </section>
      ...
      <aside class="status-card" v-if="!phoneConnected" :class="phoneConnected ? 'connected' : ''">
        ...
        <section v-else-if="connectionState === 'connected' && !phoneConnected">
          <p>Visit <code>dictaweb.netlify.com</code> on your phone and scan code:</p>
...
import { initLibsodium } from "@/utils/Crypto.js";
...
export default {
  ...
  methods: {
    ...
    async generateQRCode(string) {
      console.log(string);
      
      this.qrCodeDataUrl = await GenerateQRCode(string);
    },
    async cryptoInit() {
      this.sodium = await initLibsodium();      
      this.cryptoKey = this.sodium.to_base64(this.sodium.crypto_secretbox_keygen());
      this.generateQRCode(this.channel_id + "|" + this.cryptoKey);
    }
  },
  computed: {
    fullChannelId() {
...
$color-primary__hover: rgb(162, 79, 163);
...
$highlight-color: #3A84DA;
$highlight-input-fill-color: rgba(58, 133, 218, 0.08);

$text-color: hsl(240, 74, 23);
$form-border-color: #221122;
$form-fill-color: rgba(243, 243, 243, 0.50);

$z-layer-background: 1;
$z-layer-background-overlay: 2;
$z-layer-content: 2;
...
.button {
  ...
  &.primary {
    ...
    &:hover {
      ...
      border-color: $color-primary__hover;
      color: white;
    }
  ...
  &.danger {
    background: hsl(10, 70, 56);
    border-color:  hsl(10, 70, 36);
    color: white;

    &:hover {
      background: hsl(10, 70, 46);
      color: white;
    }
...
.status-card {
  ...
  padding: 16px;
  border-radius: 4px;
  box-shadow: 0px 4px 12px rgba(37, 37, 117, 0.2);
...
.selected-field {
  ...
  input, textarea {
    ...
    border-width: 2px;
    background: $highlight-input-fill-color;
  }
}
